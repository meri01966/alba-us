      ...
      {(
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-4 border border-blue-200">
            ...
            <div className="space-y-4">
              {EJES.map((eje) => {
                // Misma logica que el cartel de arriba (analizarAprendizaje), para que
                // la sintesis del docente SIEMPRE coincida con las bolitas y el mensaje por eje.
                const actsEje = progreso[eje.key]?.actividades || []
                const resultadosEje = actsEje.map((a: ActividadEvaluada) => a.resultado)
                const estadoEje = analizarAprendizaje(resultadosEje)
                const mensajeEje = MENSAJE_APRENDIZAJE[estadoEje]
                const tonoEje = tonoAprendizaje(estadoEje)
                ...
                return (
                  ...
                  >
                    ...
                    {estadoEje === "sin_evidencia" ? (
                      <p className="text-sm text-gray-400 italic">{mensajeEje}</p>
                    ) : (
                      <>
                        <p className="text-xs font-semibold text-gray-500 mb-1">{tonoEje.titulo}</p>
                        <p className="text-sm text-gray-600">{mensajeEje}</p>
                      </>
                    )}
                  </div>
                )
